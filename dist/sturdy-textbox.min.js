...
},localeData:function(){return this._locale},toJSON:function(){return this.toISOString()}}),moment.duration.fn.toString=moment.duration.fn.toISOString;for(i in unitMillisecondFactors)hasOwnProp(unitMillisecondFactors,i)&&makeDurationGetter(i.toLowerCase());moment.duration.fn.asMilliseconds=function(){return this.as("ms")},moment.duration.fn.asSeconds=function(){return this.as("s")},moment.duration.fn.asMinutes=function(){return this.as("m")},moment.duration.fn.asHours=function(){return this.as("h")},moment.duration.fn.asDays=function(){return this.as("d")},moment.duration.fn.asWeeks=function(){return this.as("weeks")},moment.duration.fn.asMonths=function(){return this.as("M")},moment.duration.fn.asYears=function(){return this.as("y")},moment.locale("en",{ordinalParse:/\d{1,2}(th|st|nd|rd)/,ordinal:function(number){var b=number%10,output=1===toInt(number%100/10)?"th":1===b?"st":2===b?"nd":3===b?"rd":"th";return number+output}}),hasModule?module.exports=moment:"function"==typeof define&&define.amd?(define(function(require,exports,module){return module.config&&module.config()&&module.config().noGlobal===!0&&(globalScope.moment=oldGlobalMoment),moment}),makeGlobal(!0)):makeGlobal()}.call(this),function(root,factory){"function"==typeof define&&define.amd?define([],function(){return root.returnExportsGlobal=factory()}):"object"==typeof exports?module.exports=factory():root.Formatter=factory()}(this,function(){var pattern=function(){var pattern={},DELIM_SIZE=4,regexp=new RegExp("{{([^}]+)}}","g"),getMatches=function(pattern){for(var match,matches=[];match=regexp.exec(pattern);)matches.push(match);return matches};return pattern.parse=function(pattern){var info={inpts:{},chars:{}},matches=getMatches(pattern),pLength=pattern.length,mCount=0,iCount=0,i=0,processMatch=function(val){for(var valLength=val.length,j=0;valLength>j;j++)info.inpts[iCount]=val.charAt(j),iCount++;mCount++,i+=val.length+DELIM_SIZE-1};for(i;pLength>i;i++)mCount<matches.length&&i===matches[mCount].index?processMatch(matches[mCount][1]):info.chars[i-mCount*DELIM_SIZE]=pattern.charAt(i);return info.mLength=i-mCount*DELIM_SIZE,info},pattern}(),utils=function(){{var utils={};"undefined"!=typeof navigator?navigator.userAgent:null}return utils.extend=function(destObj){for(var i=1;i<arguments.length;i++)for(var key in arguments[i])destObj[key]=arguments[i][key];return destObj},utils.addChars=function(str,chars,pos){return str.substr(0,pos)+chars+str.substr(pos,str.length)},utils.removeChars=function(str,start,end){return str.substr(0,start)+str.substr(end,str.length)},utils.isBetween=function(num,bounds){return bounds.sort(function(a,b){return a-b}),num>bounds[0]&&num<bounds[1]},utils.addListener=function(el,evt,handler){return"undefined"!=typeof el.addEventListener?el.addEventListener(evt,handler,!1):el.attachEvent("on"+evt,handler)},utils.preventDefault=function(evt){return evt.preventDefault?evt.preventDefault():evt.returnValue=!1},utils.getClip=function(evt){return evt.clipboardData?evt.clipboardData.getData("Text"):window.clipboardData?window.clipboardData.getData("Text"):void 0},utils.getMatchingKey=function(which,keyCode,keys){for(var k in keys){var key=keys[k];if(which===key.which&&keyCode===key.keyCode)return k}},utils.isDelKeyDown=function(which,keyCode){var keys={backspace:{which:8,keyCode:8},"delete":{which:46,keyCode:46}};return utils.getMatchingKey(which,keyCode,keys)},utils.isDelKeyPress=function(which,keyCode){var keys={backspace:{which:8,keyCode:8,shiftKey:!1},"delete":{which:0,keyCode:46}};return utils.getMatchingKey(which,keyCode,keys)},utils.isSpecialKeyPress=function(which,keyCode){var keys={tab:{which:0,keyCode:9},enter:{which:13,keyCode:13},end:{which:0,keyCode:35},home:{which:0,keyCode:36},leftarrow:{which:0,keyCode:37},uparrow:{which:0,keyCode:38},rightarrow:{which:0,keyCode:39},downarrow:{which:0,keyCode:40},F5:{which:116,keyCode:116}};return utils.getMatchingKey(which,keyCode,keys)},utils.isModifier=function(evt){return evt.ctrlKey||evt.altKey||evt.metaKey},utils.forEach=function(collection,callback,thisArg){if(collection.hasOwnProperty("length"))for(var index=0,len=collection.length;len>index&&callback.call(thisArg,collection[index],index,collection)!==!1;index++);else for(var key in collection)if(collection.hasOwnProperty(key)&&callback.call(thisArg,collection[key],key,collection)===!1)break},utils}(),patternMatcher=function(pattern,utils){function patternMatcher(patternSpec){var matchers=[],patterns=[];utils.forEach(patternSpec,function(patternMatcher){utils.forEach(patternMatcher,function(patternStr,matcherStr){var parsedPattern=pattern.parse(patternStr),regExpMatcher=parseMatcher(matcherStr);return matchers.push(regExpMatcher),patterns.push(parsedPattern),!1})});var getPattern=function(input){var matchedIndex;return utils.forEach(matchers,function(matcher,index){return matcher.test(input)?(matchedIndex=index,!1):void 0}),void 0===matchedIndex?null:patterns[matchedIndex]};return{getPattern:getPattern,patterns:patterns,matchers:matchers}}var parseMatcher=function(matcher){return"*"===matcher?/.*/:new RegExp(matcher)};return patternMatcher}(pattern,utils),inptSel=function(){var inptSel={};return inptSel.get=function(el){if("number"==typeof el.selectionStart)return{begin:el.selectionStart,end:el.selectionEnd};var range=document.selection.createRange();if(range&&range.parentElement()===el){var inputRange=el.createTextRange(),endRange=el.createTextRange(),length=el.value.length;return inputRange.moveToBookmark(range.getBookmark()),endRange.collapse(!1),inputRange.compareEndPoints("StartToEnd",endRange)>-1?{begin:length,end:length}:{begin:-inputRange.moveStart("character",-length),end:-inputRange.moveEnd("character",-length)}}return{begin:0,end:0}},inptSel.set=function(el,pos){if("object"!=typeof pos&&(pos={begin:pos,end:pos}),el.setSelectionRange)el.focus(),el.setSelectionRange(pos.begin,pos.end);else if(el.createTextRange){var range=el.createTextRange();range.collapse(!0),range.moveEnd("character",pos.end),range.moveStart("character",pos.begin),range.select()}},inptSel}(),formatter=function(patternMatcher,inptSel,utils){function Formatter(el,opts){var self=this;if(self.el=el,!self.el)throw new TypeError("Must provide an existing element");if(self.opts=utils.extend({},defaults,opts),"undefined"!=typeof self.opts.pattern&&(self.opts.patterns=self._specFromSinglePattern(self.opts.pattern),delete self.opts.pattern),"undefined"==typeof self.opts.patterns)throw new TypeError("Must provide a pattern or array of patterns");self.patternMatcher=patternMatcher(self.opts.patterns),self._updatePattern(),self.hldrs={},self.focus=0,utils.addListener(self.el,"keydown",function(evt){self._keyDown(evt)}),utils.addListener(self.el,"keypress",function(evt){self._keyPress(evt)}),utils.addListener(self.el,"paste",function(evt){self._paste(evt)}),self.opts.persistent&&(self._processKey("",!1),self.el.blur(),utils.addListener(self.el,"focus",function(evt){self._focus(evt)}),utils.addListener(self.el,"click",function(evt){self._focus(evt)}),utils.addListener(self.el,"touchstart",function(evt){self._focus(evt)}))}var defaults={persistent:!1,repeat:!1,placeholder:" "},inptRegs={9:/[0-9]/,a:/[A-Za-z]/,"*":/[A-Za-z0-9]/};return Formatter.addInptType=function(chr,reg){inptRegs[chr]=reg},Formatter.prototype.resetPattern=function(str){this.opts.patterns=str?this._specFromSinglePattern(str):this.opts.patterns,this.sel=inptSel.get(this.el),this.val=this.el.value,this.delta=0,this._removeChars(),this.patternMatcher=patternMatcher(this.opts.patterns);var newPattern=this.patternMatcher.getPattern(this.val);this.mLength=newPattern.mLength,this.chars=newPattern.chars,this.inpts=newPattern.inpts,this._processKey("",!1,!0)},Formatter.prototype._updatePattern=function(){var newPattern=this.patternMatcher.getPattern(this.val);newPattern&&(this.mLength=newPattern.mLength,this.chars=newPattern.chars,this.inpts=newPattern.inpts)},Formatter.prototype._keyDown=function(evt){var k=evt.which||evt.keyCode;return k&&utils.isDelKeyDown(evt.which,evt.keyCode)?(this._processKey(null,k),utils.preventDefault(evt)):void 0},Formatter.prototype._keyPress=function(evt){var k,isSpecial;return k=evt.which||evt.keyCode,isSpecial=utils.isSpecialKeyPress(evt.which,evt.keyCode),utils.isDelKeyPress(evt.which,evt.keyCode)||isSpecial||utils.isModifier(evt)?void 0:(this._processKey(String.fromCharCode(k),!1),utils.preventDefault(evt))},Formatter.prototype._paste=function(evt){return this._processKey(utils.getClip(evt),!1),utils.preventDefault(evt)},Formatter.prototype._focus=function(){var self=this;setTimeout(function(){var selection=inptSel.get(self.el),isAfterStart=selection.end>self.focus,isFirstChar=0===selection.end;(isAfterStart||isFirstChar)&&inptSel.set(self.el,self.focus)},0)},Formatter.prototype._processKey=function(chars,delKey,ignoreCaret){if(this.sel=inptSel.get(this.el),this.val=this.el.value,this.delta=0,this.sel.begin!==this.sel.end)this.delta=-1*Math.abs(this.sel.begin-this.sel.end),this.val=utils.removeChars(this.val,this.sel.begin,this.sel.end);else if(delKey&&46===delKey)this._delete();else if(delKey&&this.sel.begin-1>=0)this.val=utils.removeChars(this.val,this.sel.end-1,this.sel.end),this.delta-=1;else if(delKey)return!0;delKey||(this.val=utils.addChars(this.val,chars,this.sel.begin),this.delta+=chars.length),this._formatValue(ignoreCaret)},Formatter.prototype._delete=function(){for(;this.chars[this.sel.begin];)this._nextPos();this.sel.begin<this.val.length&&(this._nextPos(),this.val=utils.removeChars(this.val,this.sel.end-1,this.sel.end),this.delta=-1)},Formatter.prototype._nextPos=function(){this.sel.end++,this.sel.begin++},Formatter.prototype._formatValue=function(ignoreCaret){this.newPos=this.sel.end+this.delta,this._removeChars(),this._updatePattern(),this._validateInpts(),this._addChars(),this.el.value=this.val.substr(0,this.mLength),("undefined"==typeof ignoreCaret||ignoreCaret===!1)&&inptSel.set(this.el,this.newPos)},Formatter.prototype._removeChars=function(){this.sel.end>this.focus&&(this.delta+=this.sel.end-this.focus);for(var shift=0,i=0;i<=this.mLength;i++){var val,curChar=this.chars[i],curHldr=this.hldrs[i],pos=i+shift;pos=i>=this.sel.begin?pos+this.delta:pos,val=this.val.charAt(pos),(curChar&&curChar===val||curHldr&&curHldr===val)&&(this.val=utils.removeChars(this.val,pos,pos+1),shift--)}this.hldrs={},this.focus=this.val.length},Formatter.prototype._validateInpts=function(){for(var i=0;i<this.val.length;i++){var inptType=this.inpts[i],isBadType=!inptRegs[inptType],isInvalid=!isBadType&&!inptRegs[inptType].test(this.val.charAt(i)),inBounds=this.inpts[i];(isBadType||isInvalid)&&inBounds&&(this.val=utils.removeChars(this.val,i,i+1),this.focusStart--,this.newPos--,this.delta--,i--)}},Formatter.prototype._addChars=function(){if(this.opts.persistent){for(var i=0;i<=this.mLength;i++)this.val.charAt(i)||(this.val=utils.addChars(this.val,this.opts.placeholder,i),this.hldrs[i]=this.opts.placeholder),this._addChar(i);for(;this.chars[this.focus];)this.focus++}else for(var j=0;j<=this.val.length;j++){if(this.delta<=0&&j===this.focus)return!0;this._addChar(j)}},Formatter.prototype._addChar=function(i){var chr=this.chars[i];return chr?(utils.isBetween(i,[this.sel.begin-1,this.newPos+1])&&(this.newPos++,this.delta++),i<=this.focus&&this.focus++,this.hldrs[i]&&(delete this.hldrs[i],this.hldrs[i+1]=this.opts.placeholder),void(this.val=utils.addChars(this.val,chr,i))):!0},Formatter.prototype._specFromSinglePattern=function(patternStr){return[{"*":patternStr}]},Formatter}(patternMatcher,inptSel,utils);return formatter}),function(root,factory){"function"==typeof define&&define.amd?define(["jquery"],factory):root.SturdyTextbox=factory(root.jQuery)}(this,function($){"use strict";function _make(instance){_extend(instance),_addValidators(instance)}function _extend(instance){$.extend(ValidatorFactory.prototype,{settings:{prefix:"data-",selector:"[data-type],[data-required]",type:"type",format:"format",on:"on",enabled:!0,"default":"blur",defaultType:"required",pluginEnabled:!1,success:function($el){$el.addClass("valid").removeClass("invalid")},fail:function($el){$el.addClass("invalid").removeClass("valid")},error:function(e){console.log(e)}},_formatterInstances:[],_formatPatterns:[],_validators:[],_elements:[],init:function(options){options=$.extend({},instance.settings,options);var elements=$(options.selector),haveElements=elements&&elements.length>0,wireUp=instance._wireUp;haveElements&&(this._elements=elements.toArray(),this._elements.forEach(function(el){wireUp(el)}))},push:function(type,fn,format){this._validators.push(new BaseValidator(type,fn)),format&&this._formatPatterns.push({type:type,format:format})},_wireUp:function(el){var $el=el.jquery?el:$(el),events=instance._events($el),type=$el.data(instance.settings.type),defaults=instance._defaults(type),on=defaults.concat(events.filter(function(e){return defaults.indexOf(e)<0})).join(" ");$el.on(on,function(){var type=$el.data(instance.settings.type),normalize=-1===["date","time","social-security","url","email"].indexOf(type),val=normalize?$el.val().replace(/[^a-zA-Z0-9]+/g,""):$el.val();instance._validate($el,type,val)});var formatPatterns=instance._formatPatterns,formatPattern=$el.data("format")||formatPatterns[formatPatterns.map(function(item){return item.type}).indexOf(type)];formatPattern&&instance._formatterInstances.push({type:formatPattern.type,instance:new Formatter($el.get(0),{pattern:formatPattern.format,persistent:!1})})},_events:function($el){var on=$el.data(instance.settings.on);return on&&(on=on.split(",")),on||[]},_defaults:function(){return["blur"]},_validate:function($el,type,value){var settings=instance.settings,validator=instance._lookup(type);try{var valid=validator.valid(value);valid?settings.success($el):settings.fail($el)}catch(e){settings.error(e)}},_lookup:function(type){var index=instance._validators.map(function(validator){return validator.type}).indexOf(type);return instance._validators[index]}}),$.extend(BaseValidator.prototype,{valid:function(){throw new Error("Validate is not implemented.")},_isMatch:function(value,regex){return regex.test(value)}})}var _instance=null,_getInstance=function(){return _instance||(_instance=new ValidatorFactory)},ValidatorFactory=function(){},BaseValidator=function(type,valid){this.type=type,valid&&(this.valid=valid)},_addValidators=function(instance){instance.push("credit-card",function(value){return is.creditCard(value)},"{{9999}}-{{9999}}-{{9999}}-{{9999}}"),instance.push("date",function(value){return moment(value).isValid()},"{{99}}/{{99}}/{{9999}}"),instance.push("email",function(value){return is.email(value)}),instance.push("phone",function(value){return is.nanpPhone(value)||is.eppPhone(value)},"{{999}}-{{999}}-{{9999}}"),instance.push("social-security",function(value){return is.socialSecurityNumber(value)},"{{999}}-{{99}}-{{9999}}"),instance.push("time",function(value){return is.timeString(value)},"{{99}}:{{99}}:{{99}}"),instance.push("url",function(value){return is.url(value)})};return _make(_getInstance()),_getInstance()});
//# sourceMappingURL=sturdy-textbox.min.js.map